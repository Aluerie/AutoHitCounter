...
public class SKHitService(IMemoryService memoryService, HookManager hookManager) : IHitService
{
    ...
    private void InstallHitHook()
    {
        ...
        AsmHelper.WriteRelativeOffsets(bytes, [
            ...
            (code + 0xAD, WorldChrMan.Base, 7, 0xAD + 3),
            (code + 0xC7, Functions.HasSpEffectId, 5, 0xC7 + 1),
            (code + 0xD2, staggerCheckFlag, 7, 0xD2 + 2),
            (code + 0xDF, EventFlagMan.Base, 7, 0xDF + 3),
            (code + 0xEB, Functions.GetEvent, 5, 0xEB + 1),
            (code + 0xF8, hit, 6, 0xF8 + 2),
            (code + 0x102, WorldChrMan.Base, 7, 0x102 + 3),
            (code + 0x11C, Functions.HasSpEffectId, 5, 0x11C + 1),
            (code + 0x127, pendingHitFlag, 7, 0x127 + 2),
            (code + 0x12F, Hooks.Hit + 5, 5, 0x12F + 1),
        ]);
        memoryService.WriteBytes(code, bytes);
